...
function updateBackgroundPosition() {
    ...
    escada.style.left = `${escadaScreenX}px`;
}
...
const portas = [
    ...
    { nome: "Corredor 4", min: 800, max: 1050 }
];
            ...
            if (portaAtual) {
                ...
                switch (portaAtual.nome) {
                    case 'Quarto 1':
                        ...
                        setTimeout(() => {
                            window.location.href = 'duda.html';
                        }, 1000);
                        break;
                    case 'Quarto 2':
                        doorSound.play();
                        localStorage.setItem('playerX', playerX);
                        setTimeout(() => {
                            window.location.href = 'p-3-1.html';
                        }, 1000);
                        break;
                    case 'Quarto 3':
                        doorSound.play();
                        localStorage.setItem('playerX', playerX);
                        setTimeout(() => {
                            window.location.href = 'p-3-2.html';
                        }, 1000);
                        break;
                    case 'Quarto 4':
                        doorSound.play();
                        localStorage.setItem('playerX', playerX);
                        setTimeout(() => {
                            window.location.href = 'p-3-3.html';
                        }, 1000);
                        break;
                    case 'Quarto 5':
                        localStorage.setItem('playerX', playerX);
                        const jaEntrouNoQuarto5 = localStorage.getItem('quarto5_aberto') === 'true';
                        const jaTemLivroOuCerebro3 = Inventario.has('livro') || Inventario.has('cerebro3');

                        if (jaEntrouNoQuarto5 || Inventario.has('chave')) {
                            if (!jaEntrouNoQuarto5 && Inventario.has('chave')) {
                                Inventario.remove('chave');
                                localStorage.setItem('quarto5_aberto', 'true');
                            }
                            if (!jaTemLivroOuCerebro3) {
                                Inventario.add('livro');
                                alert('Parece que algo caiu, agora está no seu inventário');
                            }
                            setTimeout(() => {
                                window.location.href = 'p-3-4.html';
                            }, 1000);
                        } else {
                            alert('Você precisa da chave para entrar neste quarto.');
                        }
                        break;
                    case 'Corredor 4':
                        if (Inventario.has('cerebro3')) {
                            localStorage.setItem('parte1FoiMostrada', 'false');
                            window.location.href = 'corredor-4.html';
                        } else {
                            alert('Você precisa terminar o desafio da Duda.');
                        }
                        break;
                }
            }
