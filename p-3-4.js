...
const objectInteraction = document.getElementById('object-interaction');
const player = document.getElementById('player');
const fixedObject = document.getElementById('fixedObject');
...
const backgroundScale = 2.52;
let worldWidth = backgroundScale * window.innerWidth;

const fixedObjectWorldX = 2100;
...
function updateBackgroundPosition() {
    ...
    const fixedObjectScreenX = fixedObjectWorldX - backgroundPixelOffset;
    fixedObject.style.left = `${fixedObjectScreenX}px`;
}
...
function checkFixedObjectProximity() {
    const playerRect = player.getBoundingClientRect();
    const objectRect = fixedObject.getBoundingClientRect();
    const proximityThreshold = 100;

    return (
        playerRect.right >= objectRect.left - proximityThreshold &&
        playerRect.left <= objectRect.right + proximityThreshold &&
...
function checkInteractionDisplay() {
    if (checkDoorProximity() && !isMoving) {
        interaction.style.display = 'block';
    } else {
        interaction.style.display = 'none';
    }

    if (checkFixedObjectProximity() && !isMoving) {
        objectInteraction.style.display = 'block';
    } else {
        objectInteraction.style.display = 'none';
    }
}
document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
        ...
        case 'e':
        case 'E':
            if (checkFixedObjectProximity()) {
                objectSound.currentTime = 0;
                objectSound.play();
                objectInteraction.style.display = 'none';
            } else if (checkDoorProximity()) {
                doorSound.play();
                interaction.style.display = 'none';
                setTimeout(() => {
